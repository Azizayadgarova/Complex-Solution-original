...
// Matn animatsiyasi variantlari (reuse uchun tashqarida)
const textVariants = {
...
// Bitta blok komponenti
const ResultBlock = React.memo(({ imageSrc, imageAlt, title, description, reverse, delay }) => {
  const ref = useRef(null);
  const inView = useInView(ref, { once: true, amount: 0.3 }); // `once: true` – qayta render kamayadi
    ...
    >
      ...
      >
        <img
          src={imageSrc}
          alt={imageAlt}
          loading="lazy" // Performance optimizatsiya
      ...
      <motion.div
        ...
        className={`w-full md:w-1/2 text-[#2A5E91] text-center md:text-left ${reverse ? 'md:order-1' : 'md:order-2'}`}
      >
        <motion.h2
...
const Results = () => {
  ...
  return (
    <section className="bg-[#F7F7F7] py-10 sm:py-14 md:py-20 px-3 sm:px-5 md:px-[4%]">
