...
const VerticalStep = React.memo(({ step, title, description, isLastStep }) => {
  const ref = useRef(null);
  const inView = useInView(ref, { once: false, amount: 0.2 });
  const [isMobile, setIsMobile] = useState(typeof window !== 'undefined' ? window.innerWidth < 768 : false);

  // resize optimizatsiya (debounce + cleanup)
        ...
        <div className="flex items-start gap-4">
          ...
          <div className="flex flex-col items-center flex-shrink-0">
            <motion.div
              ...
              initial={false}
              animate={{ background: inView ? 'linear-gradient(45deg, #4CAF50, #0066CC)' : '#d1d5db' }}
    ...
    <div ref={ref} className="relative pb-24 lg:pb-32 last:pb-0">
      ...
      >
        <div className="rounded-full w-full h-full flex items-center justify-center">
          <IconComp size={window.innerWidth >= 1280 ? 28 : window.innerWidth >= 1024 ? 24 : 20} className="text-white" />
        </div>
      </motion.div>

      {/* Vertical line */}
      {!isLastStep && (
        <div className="absolute left-1/2 -translate-x-1/2 z-0" style={{ top: '48px', height: 'calc(100% - 48px)', width: '2px' }}>
          <motion.div
      ...
      <div className="grid grid-cols-[1fr_auto_1fr] gap-x-6 lg:gap-x-12 xl:gap-x-16 items-center pt-6">
        ...
        >
          <p className="text-base lg:text-lg xl:text-xl text-[#555555] leading-relaxed">{description}</p>
        </motion.div>
      </div>
    </div>
  );
